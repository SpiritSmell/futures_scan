...
class ClickhouseSaver:
    ...
    def __init__(self, url, input_queue, loop, database, table_name):
        ...
        self.handler = ClickHouseJSONHandler(self.url, database=self.database, table_name=table_name,
    ...
    async def start(self):
        ...
        while True:
            try:
                ...
                logger.info(f"[Savior] Готов к отправке данных в таблицу {self.table_name}")

                while True:
                    data = await self.input_queue.get()
                    logger.info(f"[Savior] Отправка данных: {len(data)}")
                    self.handler.insert_json_data(self.table_name, data)
            except asyncio.CancelledError:
                logger.info("[Savior] Завершение работы...")
                break
            except Exception as e:
                logger.info(
                    f"[Sender] Ошибка соединения: {e}. Повторная попытка через {MAIN_LOOP_DELAY_TIME} секунд...")
